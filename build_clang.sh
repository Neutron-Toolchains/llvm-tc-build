...
bolt_profile_gen() {
	...
	if [ "$1" = "perf" ]; then
		...
		LD_PRELOAD=/usr/lib/libjemalloc.so "$STAGE1"/perf2bolt "$STAGE3"/clang-16 \
		...
		LD_PRELOAD=/usr/lib/libjemalloc.so "$STAGE1"/llvm-bolt "$STAGE3"/clang-16 \
		...
		cd $BOLT_PROFILES
		LD_PRELOAD=/usr/lib/libjemalloc.so "$STAGE1"/merge-fdata *.fdata >combined.fdata

		LD_PRELOAD=/usr/lib/libjemalloc.so "$STAGE1"/llvm-bolt "$STAGE3"/clang-16.org \
			--data combined.fdata \
			-o "$STAGE3"/clang-16 \
			-relocs \
			-split-functions \
...
if [[ $BOLT_OPT -eq 1 ]]; then
	OPT_FLAGS_LD="-Wl,-O3,--sort-common,--as-needed,-z,now -fuse-ld=$LLVM_BIN_DIR/ld.lld -Wl,--push-state -Wl,-whole-archive -ljemalloc_pic -Wl,--pop-state -lpthread -lstdc++ -lm -ldl"
else
